...
func addRoutes(mux *http.ServeMux, sleeperClient sleeperDeps, store *db.Store, cfg config.Config, googleClient *googleauth.Client, currentWeek func(context.Context) int) {
	...
	// csrf guards the mutating POST endpoints that trust the auth cookie (or, for the two
	// unauthenticated endpoints, are cheap and standard to cover the same way) — see GH #12.
	csrf := csrfProtect(cfg.FrontendURL)
	...
	mux.Handle("POST /auth/merge", csrf(requireAuth(handlers.HandleMerge(store))))
	...
	mux.Handle("POST /league-bookmarks", csrf(optionalAuth(handlers.HandleSaveUserLeague(store))))
	...
	mux.Handle("POST /lineups", csrf(requireAuth(handlers.HandleCreateLineup(store, sleeperClient))))
	...
	mux.Handle("POST /collect", csrf(handlers.OptionalAuth(jwtSecret)(handlers.HandleCollect(store))))
