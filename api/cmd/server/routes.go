...
func addRoutes(mux *http.ServeMux, sleeperClient sleeperDeps, store *db.Store, cfg config.Config, googleClient *googleauth.Client, currentWeek func(context.Context) int) {
	...
	mux.Handle("POST /lineups", requireAuth(handlers.HandleCreateLineup(store, sleeperClient, currentWeek)))
	mux.Handle("PATCH /lineups/{id}", requireAuth(handlers.HandleUpdateLineup(store, sleeperClient, currentWeek)))
	mux.Handle("GET /lineups", optionalAuth(handlers.HandleListLineups(store, currentWeek)))
	mux.Handle("GET /lineups/{id}", handlers.HandleGetLineupByID(store, currentWeek))
	...
	mux.Handle("GET /league/{leagueId}/week/{week}", handlers.HandleGetWeekMatchups(sleeperClient, store, currentWeek))
